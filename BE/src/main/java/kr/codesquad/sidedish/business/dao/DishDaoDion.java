...
public class DishDaoDion implements DishDao {
    ...
    private final RowMapper<DishDto> dishMapper;
    private final RowMapper<BadgeDto> badgeMapper;

    public DishDaoDion(DataSource dataSource, RowMapper<DishDto> dishMapper, RowMapper<BadgeDto> badgeMapper) {
    ...
    public DishDto findDishById(Long id) {
        ...
        DishDto dishDto = namedParameterJdbcTemplate.queryForOptionalObject(DISH_DETAIL_SQL, parameters, dishMapper).orElseThrow(DishNotFoundException::new);
        ...
        dishDto.setBadges(namedParameterJdbcTemplate.query(BADGE_SQL, parameters, badgeMapper));
